...
def read_aiff_norm(filename, read_params=False):
    ...
    if read_params:
        ...
        return array, nchan, samp_wid, frame_rate, n_frames, comp_type, comp_name
    else:
        return array
...
def main(argv):
    ...
    volscale = 10 ** (float(attenuation) / 10.0)
    # read/write options
    ...
    for fi in files:
        ...
        for s in scale:
            ...
            try:
                ...
                raise ValueError(
                    "Input audio file %s is poorly formatted, exiting"
                    % os.path.basename(fi)
                )
            # scale volume
            array = array * s
            ...
            if rate != morph_samplerate:
                print(
                    "Correcting input sample rate %iHz" % (rate)
                ...
                array = change_samplerate_interp(array, float(rate), new_rate)
    ...
    if len(frame_dict) > 300 or (arrays.shape[1] / morph_samplerate) / 60.0 > 2.9:
        raise ValueError(
            "Number of splices (%i) and/or audio" % len(frame_dict)
            + " length (%2.1f minutes)" % ((arrays.shape[1] / morph_samplerate) / 60.0)
        ...
        float32_wav_file(
            outputfile, arrays, morph_samplerate, markers=frame_dict, verbose=True
        )
        print(
            "Saved Morphagene reel with %i splices: %s" % (len(frame_labs), outputfile)
        )
        if write_label_file:
            fn = os.path.splitext(os.path.basename(outputfile))[0]
            write_audacity_labels(
                "%s.txt" % fn, frame_labs / float(morph_samplerate), files
            )
