...
def onset_splice_superflux(audiofile):
    ...
    fmax = 16000.0  # highest frequency
    max_size = 3
    # Mel spectrogram
...
def retain_n_splice_markers(onset_sf, splicecount):
    ...
    if splicecount < len(onset_sf):
        splice_markers = list(
            onset_sf[i * k + min(i, m) : (i + 1) * k + min(i + 1, m)]
            for i in range(splicecount)
        )
        splice_markers[0] = [0.0]
        return np.array([x[0] for x in splice_markers])
    else:
        print(
            "More desired splices than available splices, defaulting to librosa output"
        )
        return onset_sf
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(
            argv, "hw:o:s:", ["wavfile=", "outputfile=", "splicecount="]
        )
    ...
    for opt, arg in opts:
        if opt == "-h":
            print(
                "Morphagene reel creation using Superflux onset detection:\n"
                + "morphagene_onset3.py -w <inputwavfile> -o <outputfile> -s <splicecount>\n"
    ...
    print("Number of selected splices: %i" % splicecount)
    ...
    librosa_sec = retain_n_splice_markers(
        np.unique(onset_splice_superflux(inputwavefile)), splicecount
    )
    ...
    if sample_rate != morph_srate:
        ...
        array = change_samplerate_interp(
            array, float(sample_rate), float(morph_srate)
        ).T
    ...
    if len(frame_dict) > 300 or (array.shape[1] / morph_srate) / 60.0 > 2.9:
        raise ValueError(
            "Number of splices (%i) and/or audio" % len(frame_dict)
            + " length (%2.1f minutes)" % ((array.shape[1] / morph_srate) / 60.0)
            + "exceed Morphagene limits [300 splices / 2.9 minutes]"
        )

    # write wav file with additional cue markers from labels
    write(outputfile, morph_srate, array.T.astype("float32"), markers=frame_dict)
