...
class Wave(object):
    def __init__(
        ...
        self.rate, self.data, self.bits, *all_else = wavfile.read(
            filename,
            readmarkers=readmarkers,
            readmarkerlabels=readmarkerlabels,
            readmarkerslist=readmarkerslist,
            readloops=readloops,
            readpitch=readpitch,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--readmarkers", default=False, action="store_true", help="Read markers"
    )
    parser.add_argument(
        "--readmarkerlabels",
        default=False,
        action="store_true",
        help="Read marker labels",
    )
    parser.add_argument(
        "--readmarkerslist",
        default=False,
        action="store_true",
        help="Read markers list",
    )
    parser.add_argument(
        "--readloops", default=False, action="store_true", help="Read loops"
    )
    parser.add_argument(
        "--readpitch", default=False, action="store_true", help="Read pitch"
    )
    parser.add_argument(
        "--normalized", default=False, action="store_true", help="Normalized"
    )
    parser.add_argument(
        "--forcestereo", default=False, action="store_true", help="Force stereo"
    )

    args = parser.parse_args()
    w = Wave(
        args.file[0],
        readmarkers=args.readmarkers,
        readmarkerlabels=args.readmarkerlabels,
        readmarkerslist=args.readmarkerslist,
        readloops=args.readloops,
        readpitch=args.readpitch,
        normalized=args.normalized,
        forcestereo=args.forcestereo,
    )
